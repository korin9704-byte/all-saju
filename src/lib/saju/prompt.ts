...
---
이 사람의 사주를 분석하여 정확히 13개의 주제별 사주 풀이를 작성해 주세요.
...
---
위 사주 데이터를 바탕으로 "부동산 투자로 재미 볼 수 있을까?"라는 주제로 13개 섹션으로 구성된 리포트를 작성하세요.
${concern ? `\n내담자의 궁금한 점: "${concern}"` : ""}
...
- 섹션 제목은 부동산 투자와 내담자 상황에 꼭 맞게 직접 만드세요. 매번 새롭고 구체적인 제목이어야 합니다.
- 섹션 제목에 절대 "[섹션1]", "[섹션2]" 같은 번호 레이블을 쓰지 마세요.
- 전문 용어(천간, 지지, 십성, 재성, 관성 등)는 쓰지 마세요. 꼭 써야 하면 괄호로 쉬운 설명을 붙이세요.
- 친한 선배나 상담사처럼 따뜻하고 자연스러운 말투로 쓰세요.
- 각 섹션 최소 600자 이상, 전체 5,000자 이상 작성하세요.
- [명리학 근거 필수] 각 섹션에서 반드시 사주 데이터 근거를 자연스럽게 본문에 녹이세요. 예: "지금 흘러오는 대운의 기운이 부동산 투자와 직접 맞닿아 있어요", "타고난 사주의 이런 구조 때문에 이 투자 성향이 나오는 거예요", "올해 세운이 당신 사주의 재물 기운을 건드리고 있어서". 단순히 "좋아요 / 안 좋아요"로만 끝내지 마세요.
${concern ? "- 내담자의 궁금한 점에 대한 답변을 13개 섹션 안에 자연스럽게 녹여서 작성하세요." : ""}

출력 형식 (반드시 이 구조 그대로):
## [부동산 투자 주제에 맞게 직접 만든 섹션 제목 1]

[600자 이상 본문. 소제목 없이 문단으로만.]

## [부동산 투자 주제에 맞게 직접 만든 섹션 제목 2]
...
// ─── 이성 인연 풀이 (romance-saju) ──────────────────────────────────────────
export function buildRomanceSajuPrompt(input: PromptInput): { system: string; user: string } {
  const ctx = makeContext(input);
  const concern = input.concerns
    .map(c => c.replace(/^\[질문\]\s*/, ""))
    .filter(Boolean)
...
---
위 사주 데이터를 바탕으로 "이성이 많을 인생인가?"라는 주제로 13개 섹션으로 구성된 리포트를 작성하세요.
${concern ? `\n내담자의 궁금한 점: "${concern}"` : ""}
...
- 각 섹션은 반드시 ## 으로 시작하는 제목 1줄 + 본문으로만 구성합니다. 섹션 안에 소제목(###, **굵은글씨** 등)을 절대 쓰지 마세요.
- 본문은 문단(paragraph)으로만 씁니다. 소제목, 볼드 제목, 리스트 없이 순수 텍스트 문단만 사용하세요.
- 모든 섹션은 이성 인연·매력·연애 운·이성 복 주제에 초점을 맞추세요.
- 섹션 제목은 이성 인연과 내담자 상황에 꼭 맞게 직접 만드세요. 매번 새롭고 구체적인 제목이어야 합니다.
- 섹션 제목에 절대 "[섹션1]", "[섹션2]" 같은 번호 레이블을 쓰지 마세요.
- 전문 용어(천간, 지지, 십성, 재성, 관성 등)는 쓰지 마세요. 꼭 써야 하면 괄호로 쉬운 설명을 붙이세요.
- 친한 선배나 상담사처럼 따뜻하고 자연스러운 말투로 쓰세요.
- 각 섹션 최소 600자 이상, 전체 5,000자 이상 작성하세요.
- [명리학 근거 필수] 각 섹션에서 반드시 사주 데이터 근거를 자연스럽게 본문에 녹이세요. 예: "타고난 사주 기운이 이성을 끌어당기는 구조예요", "지금 흘러오는 대운이 이성 인연이 활발해지는 시기예요", "올해 세운이 당신 사주의 이성 운을 건드리고 있어서". 단순히 "좋아요 / 안 좋아요"로만 끝내지 마세요.
${concern ? "- 내담자의 궁금한 점에 대한 답변을 13개 섹션 안에 자연스럽게 녹여서 작성하세요." : ""}

출력 형식 (반드시 이 구조 그대로):
## [이성 인연 주제에 맞게 직접 만든 섹션 제목 1]

[600자 이상 본문. 소제목 없이 문단으로만.]

## [이성 인연 주제에 맞게 직접 만든 섹션 제목 2]
...
// ─── 직무 적성 풀이 (job-saju) ───────────────────────────────────────────────
export function buildJobSajuPrompt(input: PromptInput): { system: string; user: string } {
  const ctx = makeContext(input);
  const concern = input.concerns
    .map(c => c.replace(/^\[질문\]\s*/, ""))
    .filter(Boolean)
...
---
위 사주 데이터를 바탕으로 "나는 어떤 직무가 맞을까?"라는 주제로 13개 섹션으로 구성된 리포트를 작성하세요.
${concern ? `\n내담자의 궁금한 점: "${concern}"` : ""}
...
- 각 섹션은 반드시 ## 으로 시작하는 제목 1줄 + 본문으로만 구성합니다. 섹션 안에 소제목(###, **굵은글씨** 등)을 절대 쓰지 마세요.
- 본문은 문단(paragraph)으로만 씁니다. 소제목, 볼드 제목, 리스트 없이 순수 텍스트 문단만 사용하세요.
- 모든 섹션은 직무·적성·커리어·일 주제에 초점을 맞추세요.
- 섹션 제목은 직무 적성과 내담자 상황에 꼭 맞게 직접 만드세요. 매번 새롭고 구체적인 제목이어야 합니다.
- 섹션 제목에 절대 "[섹션1]", "[섹션2]" 같은 번호 레이블을 쓰지 마세요.
- 전문 용어(천간, 지지, 십성, 재성, 관성 등)는 쓰지 마세요. 꼭 써야 하면 괄호로 쉬운 설명을 붙이세요.
- 친한 선배나 상담사처럼 따뜻하고 자연스러운 말투로 쓰세요.
- 각 섹션 최소 600자 이상, 전체 5,000자 이상 작성하세요.
- [명리학 근거 필수] 각 섹션에서 반드시 사주 데이터 근거를 자연스럽게 본문에 녹이세요. 예: "타고난 사주 기운이 이런 직무 스타일을 만들어요", "지금 흘러오는 대운이 커리어 전환과 맞닿아 있어요", "올해 세운이 당신의 직업 운을 건드리고 있어서". 단순히 "좋아요 / 안 좋아요"로만 끝내지 마세요.
${concern ? "- 내담자의 궁금한 점에 대한 답변을 13개 섹션 안에 자연스럽게 녹여서 작성하세요." : ""}

출력 형식 (반드시 이 구조 그대로):
## [직무 적성 주제에 맞게 직접 만든 섹션 제목 1]

[600자 이상 본문. 소제목 없이 문단으로만.]

## [직무 적성 주제에 맞게 직접 만든 섹션 제목 2]
...
// ─── 사업 운 풀이 (business-saju) ───────────────────────────────────────────
export function buildBusinessSajuPrompt(input: PromptInput): { system: string; user: string } {
  const ctx = makeContext(input);
  const concern = input.concerns
    .map(c => c.replace(/^\[질문\]\s*/, ""))
    .filter(Boolean)
...
---
위 사주 데이터를 바탕으로 "나는 사업해도 되는 사주일까?"라는 주제로 13개 섹션으로 구성된 리포트를 작성하세요.
${concern ? `\n내담자의 궁금한 점: "${concern}"` : ""}
...
- 각 섹션은 반드시 ## 으로 시작하는 제목 1줄 + 본문으로만 구성합니다. 섹션 안에 소제목(###, **굵은글씨** 등)을 절대 쓰지 마세요.
- 본문은 문단(paragraph)으로만 씁니다. 소제목, 볼드 제목, 리스트 없이 순수 텍스트 문단만 사용하세요.
- 모든 섹션은 사업·창업·독립·사장 기질 주제에 초점을 맞추세요.
- 섹션 제목은 사업 운과 내담자 상황에 꼭 맞게 직접 만드세요. 매번 새롭고 구체적인 제목이어야 합니다.
- 섹션 제목에 절대 "[섹션1]", "[섹션2]" 같은 번호 레이블을 쓰지 마세요.
- 전문 용어(천간, 지지, 십성, 재성, 관성 등)는 쓰지 마세요. 꼭 써야 하면 괄호로 쉬운 설명을 붙이세요.
- 친한 선배나 상담사처럼 따뜻하고 자연스러운 말투로 쓰세요.
- 각 섹션 최소 600자 이상, 전체 5,000자 이상 작성하세요.
- [명리학 근거 필수] 각 섹션에서 반드시 사주 데이터 근거를 자연스럽게 본문에 녹이세요. 예: "타고난 사주 기운이 사장 체질인지 직원 체질인지를 보여줘요", "지금 흘러오는 대운의 기운이 창업 타이밍과 맞닿아 있어요", "올해 세운이 당신 사주의 사업 기운을 건드리고 있어서". 단순히 "좋아요 / 안 좋아요"로만 끝내지 마세요.
${concern ? "- 내담자의 궁금한 점에 대한 답변을 13개 섹션 안에 자연스럽게 녹여서 작성하세요." : ""}

출력 형식 (반드시 이 구조 그대로):
## [사업 운 주제에 맞게 직접 만든 섹션 제목 1]

[600자 이상 본문. 소제목 없이 문단으로만.]

## [사업 운 주제에 맞게 직접 만든 섹션 제목 2]
...
// ─── 분할 호출 (레거시 — 현재 미사용) ──────
// ─── 무료 미니 사주 (free-mini — 공유받은 친구용 3섹션 티저) ──────────
export function buildMiniSajuPrompt(input: PromptInput): { system: string; user: string } {
  const ctx = makeContext(input);

  const user = `${ctx}

---
이 사람의 사주를 분석하여 가볍게 읽는 "미니 사주" 3개 섹션을 작성해 주세요.

⚠️ 필수 규칙:
- 섹션은 정확히 3개만 작성하세요.
- 각 섹션 제목은 마크다운 ## 형식이며, 시적이고 개성 있는 문장으로 쓰세요. "성격", "운세" 같은 단순 제목 금지.
- 각 섹션 본문은 300~450자로 짧고 임팩트 있게 쓰세요. 긴 설명 대신 "어? 내 얘기 같은데" 싶은 구체적 포인트 위주로.
- 3개 섹션은 순서대로 ① 타고난 기질의 핵심 ② 지금 들어와 있는 운의 흐름 ③ 올해 챙기면 좋은 것 한 가지 를 다루세요.
- 전문 용어 없이 일상 언어로, 친한 선배처럼 따뜻하게 쓰세요.
- [명리학 근거 필수] 각 섹션 첫 문장에서 사주 근거를 자연스럽게 언급하세요.
- 마지막 섹션 끝에 "여기까지는 맛보기예요. 더 깊은 이야기가 궁금하다면 전체 풀이에서 만나요." 같은 한 줄을 자연스럽게 붙이세요.
...
- 주제는 반드시 정확히 13개를 작성하세요. 12개도 14개도 안 됩니다. 13개보다 많거나 적으면 절대 안 됩니다. 반드시 딱 13개.
- 각 주제 제목은 마크다운 ## 형식으로 작성하세요.
- 제목은 반드시 시적이고 개성 있는 문장으로 작성하세요. 예: "겉은 고요한 호수지만 속은 용암이 끓어오르는 외유내강의 결정체". 절대 "성격 분석", "고민 답변" 같은 단순한 제목은 쓰지 마세요.
- [고민 집중 필수] 13개 주제 중 앞쪽 절반 이상은 내담자의 고민을 정면으로 다루세요. 고민의 원인이 사주 어디에서 오는지, 지금 운의 흐름이 고민과 어떻게 맞물리는지, 언제쯤 어떻게 풀리는지, 무엇을 하면 좋은지를 구체적으로 서술하세요. 나머지 주제(성격/재능/재물/인간관계 등)도 반드시 고민과 연결 지어 서술하세요.
- [해결 확률 필수] 첫 번째 주제 본문 안에서 고민의 결과 가능성을 반드시 구체적인 퍼센트 숫자로 제시하세요. 단, "이 고민이 해결될 가능성은 N%예요" 같은 고정 문구를 쓰지 말고, 반드시 내담자의 고민 내용에 딱 맞는 문장으로 표현하세요. 예: 이직 고민이면 "올해 안에 더 좋은 자리로 옮길 가능성은 74%예요", 재물 고민이면 "하반기에 수익을 손에 쥘 가능성은 68%예요", 연애 고민이면 "이 인연이 이어질 가능성은 61%예요", 시험 고민이면 "이번에 합격할 가능성은 77%예요". 사주와 운의 흐름을 근거로 산출한 숫자여야 하며, 왜 그 확률인지 이유도 함께 서술하세요. 두루뭉술하게 "높아요/낮아요"로만 쓰는 것은 금지합니다.
- [풀리는 시기 필수] 첫 번째 주제 본문 안에서 고민의 흐름이 바뀌는 시기를 반드시 구체적인 연도와 월(또는 분기)로 제시하세요. 이 역시 "풀리는 시기는 ~예요" 같은 고정 문구 대신 고민 내용에 맞는 자연스러운 문장으로 쓰세요. 예: "이직 문이 크게 열리는 건 2026년 10월쯤이에요", "두 사람 사이가 결정 나는 건 2027년 봄이에요". 현재 날짜 이후의 시기여야 하며, 대운·세운 흐름을 근거로 왜 그 시기인지 이유도 함께 서술하세요.
- 각 주제 본문은 반드시 800자 이상 작성하세요. 그 이하로 쓰면 안 됩니다.
- 전문 용어(천간, 지지, 십성, 재성 등)는 쓰지 말고 일상 언어로 풀어 설명하세요.
- 친한 선배처럼 따뜻하고 자연스러운 말투로 쓰세요. 고민을 가볍게 넘기지 말고, 충분히 공감한 뒤 사주 근거로 방향을 제시하세요.
- 마크다운 ## 헤딩만 사용하세요 (###, ####는 쓰지 마세요).
- [명리학 근거 필수] 각 주제 본문 첫 문단에서 반드시 이 해석의 사주 근거를 자연스럽게 언급하세요. 예: "태어날 때부터 불의 기운이 사주 곳곳에 자리 잡고 있어서", "현재 흘러오는 대운의 흐름이 이 부분을 강하게 건드리고 있어서". 근거 없이 결론만 쓰는 것은 금지합니다.
