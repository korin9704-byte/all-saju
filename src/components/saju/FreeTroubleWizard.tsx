...
export function FreeTroubleWizard({ productId }: { productId: string }) {
  ...
  const numInputCls = "w-full bg-white border border-[#E7DDF8] rounded-2xl px-4 py-3 text-sm text-[#4A3A72] text-center placeholder:text-[#4A3A72]/35 focus:outline-none focus:border-[#C95FC0] transition-colors disabled:opacity-40";
  const textInputCls = "w-full bg-white border border-[#E7DDF8] rounded-2xl px-4 py-3 text-sm text-[#4A3A72] placeholder:text-[#4A3A72]/35 focus:outline-none focus:border-[#C95FC0] transition-colors";
  ...
  const radioRow = (selected: boolean, label: string, onClick: () => void, key?: string) => (
    <label key={key ?? label}
      className={`flex items-center gap-3 cursor-pointer rounded-2xl px-4 py-3 transition-colors ${selected ? "bg-[#E7DDF8] border border-[#C95FC0]" : "bg-white border border-[#E7DDF8]"}`}>
      <input type="radio" checked={selected} onChange={onClick} className="w-4 h-4 accent-[#C95FC0]" />
          ...
          {STEPS.map((s, i) => (
            <div
              ...
              style={{
                ...
                backgroundColor: i === stepIdx ? "#C95FC0" : "#E7DDF8",
              }}
            />
          ))}
        </div>
            ...
            <>
              ...
              <div className="relative mb-8">
                <textarea value={concern} rows={6}
                  ...
                  className="block w-full resize-none rounded-2xl bg-white border border-[#E7DDF8] px-5 py-4 text-sm text-[#4A3A72] leading-relaxed placeholder:text-[#4A3A72]/35 focus:outline-none focus:border-[#C95FC0] transition-colors" />
