...
export default async function ProductDetailPage({
  ...
  const productDescriptions: Record<string, React.ReactNode[]> = {
    "today-fortune": [
      <>타고난 성격과 기질부터 올해의 운세 흐름까지, 내 사주를 {hl("누구나 이해할 수 있는 쉬운 말")}로 풀어드려요. 한자나 전문 용어 없이, 읽는 순간 '이게 나잖아' 싶은 느낌이 드실 거예요. 어떤 분이 봐도 {hl("'내 이야기 같다'")}는 반응이 나오는 이유가 있어요.</>,
      <>직업·재물·연애·건강·인간관계까지 {hl("삶 전반을 한눈에")} 살펴보고, 올해 내가 어떤 흐름 속에 있는지, {hl("유리한 시기와 조심해야 할 시기")}는 언제인지 구체적으로 안내해드려요. 단순한 운세 풀이가 아닌, {hl("내 삶의 패턴과 타고난 강점")}, 반복되는 시련의 이유까지 함께 짚어드려요.</>,
      <>읽고 나면 지금까지의 내 인생이 왜 이렇게 흘러왔는지 이해가 되기 시작하고, 앞으로 어떻게 나아가면 좋을지 방향이 보일 거예요. 사주를 처음 접하는 분도, 나를 더 깊이 이해하고 싶은 분도 모두 강력 추천드려요.</>,
    ],
    "premium-saju": [
      <>다가올 {hl("10년 대운의 흐름을 연도별로")} 상세히 분석해드려요. 인생에는 치고 나가야 할 때와 조용히 준비해야 할 때가 분명히 따로 있어요. 많은 사람들이 이걸 모르고 쉬어야 할 때 무리하고, 나아가야 할 때 머뭇거리다가 기회를 놓쳐요.</>,
      <>대운의 흐름을 미리 알면, {hl("에너지를 써야 할 시기와 기다려야 할 시기")}를 현명하게 선택할 수 있어요. 언제 새로운 도전을 시작하고, 언제 내실을 다져야 할지 큰 그림을 잡아드려요. 앞으로 어떤 파도가 오는지 미리 알고 준비하면 {hl("같은 상황에서도 결과가 완전히 달라질 수 있어요.")}</>,
      <>{hl("이직·창업·결혼·투자")} 등 인생의 중요한 결정을 앞두고 있다면 특히 도움이 돼요. 잘못된 타이밍에 움직여서 고생하는 일이 없도록, 내 인생의 황금기를 절대 놓치지 마세요.</>,
    ],
    "love-saju": [
      <>두 사람의 사주를 나란히 놓고 궁합 점수와 함께 {hl("감정·소통·갈등·가치관·결혼 궁합까지 13가지 주제")}로 꼼꼼히 분석해요. 단순한 점수 하나가 아닌, {hl("두 사람 사이에서 실제로 일어나는 일들")}을 사주로 구체적으로 풀어드려요.</>,
      <>왜 이 사람과 {hl("자꾸 같은 패턴의 갈등이 반복")}되는지, 서로 잘 맞는 부분과 보완이 필요한 부분은 어디인지 짚어드려요. 어떻게 대화하면 더 잘 통할 수 있는지, {hl("함께 더 행복해질 수 있는 방향")}까지 안내해드릴게요. 우리가 왜 이런 관계인지, 사주를 통해 보면 훨씬 선명하게 보여요.</>,
      <>연인·부부·짝사랑·친구·가족 모두 가능해요. 관계가 답답하게 느껴질 때, 상대방을 더 깊이 이해하고 싶을 때, 우리가 잘 맞는지 궁금할 때 꼭 한번 확인해보세요.</>,
    ],
    "worry-saju": [
      <>연애·직장·돈·관계 등 {hl("어떤 질문이든")} 내 사주와 연결해 구체적으로 답해드려요. 깊은 고민일 수도, 문득 떠오른 궁금증일 수도 있어요. 거창하지 않아도 괜찮아요.</>,
      <>사주의 흐름을 통해 {hl("왜 이런 상황이 생겼는지 맥락")}부터 짚어드려요. 지금 내가 어떤 시기에 있는지 먼저 이해하면, 같은 상황도 완전히 다르게 보이기 시작해요. 두루뭉술한 위로가 아닌, {hl("내 사주에 맞는 맞춤형 해설")}로 지금 당장 어떻게 행동하면 좋을지 방향을 잡아드릴게요.</>,
      <>다른 사람에게 물어보기 애매했던 것도, 혼자 끙끙 앓아온 것도 괜찮아요. {hl("질문을 구체적으로 적어주실수록 더 정확하고 실질적인 답변")}을 드릴 수 있어요. 편하게 무엇이든 물어보세요.</>,
    ],
  };
  ...
  return (
    <div className="container py-12 max-w-lg">
      ...
      {productDescriptions[product.slug] && (
        <div className="mb-8 space-y-3 text-sm text-body leading-relaxed">
